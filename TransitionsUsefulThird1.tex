...
$\wt(c,a)$ & State cache $c$ is waiting to upgrade to for address $a$, or $\epsilon$\\ %Gives cache $c$'s desired coherence state to upgrade to (for address $a$), or $\epsilon$ if none\\
$\dwt(p,c,a)$ & State cache $p$ is waiting its child $c$ to downgrade to for address $a$, or $\epsilon$\\
...
\end{lemma}
\begin{defn}
\textit{compatible}: States $x_1$ and $x_2$ are \textit{compatible}
