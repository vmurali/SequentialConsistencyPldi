...
\inference
[Load]
{\inp(i) = \mathit{qs} \uplus \{t, \ld,a\}}
{\io{$M_m$}{\inp, \outp, m}{i, \inp[i \coloneqq \mathit{qs}], \outp[i \coloneqq
(t, \ld, m(a)) \uplus \outp(i)], m}{}}
...
\inference[LdRepGood]
{\commit(\rob) = (\pc,\pc',(\ld,x,a,v,s'))}
{\iol{\text{P$_\text{so}$}}{s,\pc,\top,\rob,\ppc}{s',\pc',\bot,\retire(\rob),\ppc}{\epsilon, \ld,v}}

\inference[LdRepBad]
{\commit(\rob) = (\pc,\pc',(\ld,x,a,v',s')) & v' \neq v & \exec(s,\pc,(\ld,x,v)) = (\pc'', s'')}
{\iol{\text{P$_\text{so}$}}{s,\pc,\top,\rob,\ppc}{s'',\pc'',\bot,\phi,\setBp(\ppc,\pc)}{\epsilon, \ld, v}}
...
\end{corollary}
\begin{proof}
Direct consequence of Theorems~\ref{ocorrect},~\ref{liftn} (where the
