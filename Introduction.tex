...
implementing such systems, to such an extent that they have already become some
...
modular boundary to be realized by multiple implementation any of which may be
safely dropped into the system. It also allows a greater measure of
parallelization of design improving development time. Similarly verification is
simplified as modular interface agreements provide a set of natural lemmas to
verify. This leads us towards a decomposition of the whole system verification
task into lemmas of sub-systems, which can be composed in a black-box manner to
produce full-system theorems.

This works introduces an interface enabling true modular implementation and
...
Our proof methodology involves describing a reference model of a component and
an implementation model for that component using labeled transition systems
(LTS) and formally proving that the implementation is correct (sound) with
respect to our reference. Using LTS for describing hardware has already been
studied \cite{HoeArvind:TRSSynthesis1, Hoe:TCAD}, and there are compilers converting an LTS into efficient
hardware \cite{BSV:LangRef, Bluespec:TFRG}. In fact, a cache coherence protocol implementation in Bluespec was discussed in \cite{DNA:CoherenceImplementation}. In fact, the cache coherence protocol that we
study in this work is obtained from a direct transliteration of Bluespec code
used to implement a multi-processor PowerPC system \cite{Khan:PowerPc}. The
resulting implementation from that Bluespec code is pretty efficient; an 8-core
system can run 55 million instructions overall per second on the BEE FPGA
board, with a 2-level cache hierarchy. All our proofs are mechanized in the Coq
proof assistant, and are included as the non-anonymous supplement to this
paper. 
...
decomposition of any multi-processor system independent of the memory model
that it implements, and discuss the store-atomicity property of the memory
...
complex system and a discussion of related work and other conclusions in
