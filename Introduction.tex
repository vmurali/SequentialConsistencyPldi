...
%Proofs in this style are quite common in the software world.  The function
%preconditions and postconditions of Hoare logic provide a canonical
%example, and there have been many innovations in modular proof since Hoare's
%original work.  Somehow the hardware verification world has taken less
%advantage of modularity.  In this paper, we propose a new program in
%hardware verification, based on modular proofs using the formalism of
%\emph{labelled transition systems} that is well-known in process-calculus
%circles.  It turns out to be exactly the right way of capturing contracts on
...
In each instance we describe a reference model and an implementation model for
...
As a side effect, we %debunk a common misconception about the semantics of the
...
The Coq source code of our proofs is included as the non-anonymous
